...
use crate::utils::variables::{CONFIG, HOST, LOCAL_STORAGE_PATH, USE_S3};
...
async fn main() -> std::io::Result<()> {
    ...
    config::Config::init().unwrap_or_else(|err| panic!("Unable to load the config '{}'. {}", *CONFIG, err));

    info!("Starting Winter server.");
    ...
    HttpServer::new(|| {
        App::new()
            ...
            .route("/{tag:[^/]*}/download/{filename:.*}", web::get().to(routes::download::get))
            .route("/{tag:[^/]*}/{filename:[^/]*}", web::get().to(routes::serve::get))
            .route("/{tag:[^/]*}/{filename:[^/]*}/{fn:.*}", web::get().to(routes::serve::get))
